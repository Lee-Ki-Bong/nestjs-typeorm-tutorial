...
- 이전 예시 중에 DTO 객체를 save() 매개변수에 넘기고 있는 부분이 있었다. 제대로 동작이 이루어지는 이유는 DTO 객체와 엔티티의 구조가 동일하기 때문이다. 그러나 이 행위는, DTO객체가 그대로 사용되고 TypeORM은 해당 DTO 객체를 데이터베이스에 저장하고 있는 것이다.
